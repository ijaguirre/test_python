...
def mover_piezaas(tablero, x1, y1, x2, y2):
    ...
    elif x1>x2 and y1>y2 and ((x1-x2)==(y1-y2)) and mm == True: #############De verdad esta lista
        print ("entro")
        p = x1-x2
        mm = check_pieza(tablero,x2,y2)
        print("x1>x2 and y1>y2")
    ...
    elif x1>x2 and y1<y2 :#Muy bien, es para moverse de 3,5 a 1,7 --> x ahi va!
        print("x1>x2 and y1+1<y2")
        s = 1
        mm = check_pieza(tablero, x2, y2)
        while x1-s>x2 and y1+s <y2 and (x1-x2==s) and (y2-y1==s): #como nunca entra retorna true igual
...
print ( "chuc",check_pieza(tablero_juego, 7,1 ))
while turno > 0:  # Falta condicion que ninguno este haya perdido, pero quedara pendiente. y debe ir con un if final
    if turno % 2 != 0:
        ...
        if que_hacer == "1":#Dos ciclos, una para que lo ingreso correctamente y otro para que sea "X"
            ...
            if movimiento == True:
                ...
                if rocio == True:
                    ...
                    nacho =mover_piezaas(tablero_juego,fila_nueva, columna_nueva, fila_flecha, columna_flecha)
                    print (nacho)
